          ...
          >
            ...
            <template v-if="!showOtp">
              ...
              >
                ...
                <!-- STEP 1: معلومات المنشأة -->
                <!-- اسم المنشأة -->
                <BaseInput
                  label="اسم المنشأة"
                  v-model="form.info.orgName"
              ...
              >
                ...
                <BaseInput
                  label="اسم المنشأة"
                  v-model="form.office.orgName"
                  :error="errors.officeOrgName"
                  placeholder="مثال: مكتب الاستشارات المتقدّم"
                />

                <!-- النشاط -->
                <BaseInput
                  label="النشاط"
                  v-model="form.office.activity"
                  :error="errors.officeActivity"
                  placeholder="مثال: استشارات أعمال"
                />
                ...
                <BaseInput
                  label="رقم الرخصة"
                  v-model="form.office.licenseNo"
                  :error="errors.officeLicenseNo"
                  placeholder="مثال: 1234567890"
                  inputmode="numeric"
                />
                ...
                <BaseInput
                  label="اسم المدير"
                  v-model="form.office.manager"
                  :error="errors.officeManager"
                  placeholder="مثال: أحمد العتيبي"
                />
                ...
                <BaseInput
                  class="md:col-span-2"
                  label="الرقم السري"
                  v-model="form.office.password"
                  :error="errors.officePassword"
                  type="password"
                  placeholder="••••••••"
                />
              </div>

              <!-- STEP 3: بيانات المنشأة -->
              <div
                v-else-if="step === 2"
                class="grid grid-cols-1 gap-4 md:grid-cols-2"
              >
                ...
                <BaseFile
                  label="مرفق السجل"
                  v-model="form.org.crAttachment"
                  :error="errors.crAttachment"
                ...
                <BaseInput
                  label="رقم المنشأة"
                  v-model="form.org.orgNo"
                  :error="errors.orgNo"
                  inputmode="numeric"
                />

                <!-- المرفق (خاص برقم المنشأة) -->
                <BaseFile
                  label="المرفق"
                  v-model="form.org.orgAttachment"
                  :error="errors.orgAttachment"
                  accept=".pdf,.jpg,.jpeg,.png"
                />
                ...
                <BaseInput
                  label="الرقم الموحد"
                  v-model="form.org.unifiedNo"
                  :error="errors.unifiedNo"
                  inputmode="numeric"
                />

                <!-- المرفق (خاص بالرقم الموحد) -->
                <BaseFile
                  label="المرفق"
                  v-model="form.org.unifiedAttachment"
                  :error="errors.unifiedAttachment"
                  accept=".pdf,.jpg,.jpeg,.png"
                />
            ...
            <template v-else>
              <div class="max-w-xl mx-auto text-center">
                ...
                <p class="mb-6 text-sm text-slate-600 dark:text-slate-300">
                  ...
                  <span class="font-semibold text-primary">{{
                    form.info.mobile || form.info.email
                  }}</span>
                </p>
...
export default {
  ...
  data() {
    return {
      step: 0,
      steps: [
        { key: "info", title: "معلومات المنشأة " },
        { key: "office", title: "بيانات مكتب الاستشارات" },
      ...
      form: {
        ...
        org: {
          ...
          orgNo: "", // رقم المنشأة
          orgAttachment: null, // المرفق
          unifiedNo: "", // الرقم الموحد
          unifiedAttachment: null, // المرفق
          taxNo: "", // الرقم الضريبي
        },
  ...
  computed: {
    ...
    // one v-model that switches between info.type and consultant.type
    selectedTypeModel: {
      get() {
        return this.isClient
          ? this.form.info?.type ?? ""
      ...
      set(val) {
        if (this.isClient) this.form.info.type = val;
